...
static void OnPlaneSegmentOneExist(const Vector3D& A, const Vector3D& B, double alpha, double beta) {
    if (A.is_parallel_to(B)) return;
    TestOneExist(A * (1 - alpha), A * alpha, B * (1 - beta), B * beta, Vector3D(0, 0, 0));
}
...
TEST(OnePlaneSegment, OneExist) {
    ...
    OnPlaneSegmentOneExist(Vector3D(1, 0, 1e-4), Vector3D(7, 0, 1e4), 2.1, 1);
    OnPlaneSegmentOneExist(Vector3D(1, 1e4, 0), Vector3D(0, 1e-4, 1.9), 2.1, 0.1);
    OnPlaneSegmentOneExist(Vector3D(1, 23, 45), Vector3D(1e4, 1e-4, 0),  0, 0.1);
}

TEST(OnePlaneSegment, NotExist) {    
